...
def test_najia_divination():
    ...
    print("\n1. 自动起卦测试:")
    try:        result = generate_najia_divination_for_api(
            question="测试纳甲功能是否正常",
        ...
        # Print line details
        print("\n六爻详情:")
        for i, line in enumerate(result.original_hexagram.lines):
            shi_ying = ""
            if line.shi_yao:
                shi_ying = " [世]"
            elif line.ying_yao:
    ...
    print("\n2. 手动爻象测试:")
    try:
        manual_lines = [7, 8, 9, 6, 7, 8]  # 少阳、少阴、老阳(变)、老阴(变)、少阳、少阴        result = generate_najia_divination_for_api(
            question="手动输入爻象测试",
            manual_lines=manual_lines
        )
        
        print(f"手动爻象: {manual_lines}")
        print(f"本卦: {result.original_hexagram.name}")
        
        changing_lines = [line for line in result.original_hexagram.lines if line.changing]
        if changing_lines:
            print(f"变爻: {[line.position for line in changing_lines]}")
        
        if result.changed_hexagram:
            print(f"变卦: {result.changed_hexagram.name}")
    ...
    print("\n3. 解卦测试:")
    try:
        interpretation = generate_najia_interpretation(result)
